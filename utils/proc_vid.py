...
class VID(object):
    def __init__(video_path):
        ...
        self.width = np.int32(self.vidcap.get(cv2.CAP_PROP_FRAME_WIDTH))  # float
        self.height = np.int32(self.vidcap.get(cv2.CAP_PROP_FRAME_HEIGHT))  # float
        self.frame_num = int(self.vidcap.get(cv2.CAP_PROP_FRAME_COUNT))
